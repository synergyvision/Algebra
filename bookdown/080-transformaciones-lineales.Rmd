...
$$v=(v_{1}, v_{2},\cdots , v_{n}) \left( \begin{array}{c}
 ...
 \vdots\\
 \lambda_{n}
\end{array} \right)$$ Se suele denotar $[v]_{\mathcal{B}}$ a *la matriz de coordenadas de $v$ en la base ordenada* $\mathcal{B}$ y puede mostrarse como una matriz fila (un vector) en lugar de una matriz columna, por comodidad.
...
```{example}
	...
	$$P=\left(\begin{array}{cc}
		 ...
		 -\frac{2}{5} & \frac{1}{5}
		 \end{array} \right)$$ 
y así, la matriz cambio de base de $\mathcal{B}_{2}$ a la base $\mathcal{B}_{1}$ es la inversa de $P$, esto es  $$P^{-1}=\left(\begin{array}{cc}
		 1 & -2\\
		 2 & 1
	\end{array} \right)$$
...
```{example}
  ...
  es una matriz invertible, cuya inversa es 
	...
	-\sen\theta & \cos\theta
	\end{array} \right)$$
	Luego $\mathcal{B}_{2}=\{(\cos\theta,\sen\theta),(-\sen\theta,\cos\theta)\}$ 
...
```{theorem}
	Sean $V$ y $W$ espacios vectoriales, donde $V$ es de dimensión finita. Sea $T$ una transformación lineal de $V$ en $W$. Entonces $$dim V=rango(T)+Nul(T).$$
...
```{proof}
	...
	Definamos la siguiente familia de transformaciones lineales: $E^{pq}(v_{1}): V\longrightarrow W$ como $$E^{pq}(v_{j})=\left\lbrace \begin{array}{ll}
	0& \mbox{ si } j\neq q\\
	u_{p}& \mbox{ si } j=q
