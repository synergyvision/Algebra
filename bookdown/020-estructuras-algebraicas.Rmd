...
```{remark}
Dado un elemento $f\in S_{n}$, podemos representarlo por $\left(  {\begin{array}{ccccc}
	1 & 2 & 3 & \cdots & n \\
...
```{remark}
	Si dos de estos ?ndices son finitos, tambi?n lo ser? el tercero.
...
```{remark}
	(i) Todo dominio de integridad y todo anillo con división tiene al menos dos elementos, denotados $0$ y $1_{A}$.
...
```{definition}
	Un subconjunto no vacío $I$ de $A$ se dice que es un ideal (bilateral) de $A$ si:
